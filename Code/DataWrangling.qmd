...
Cytokine_Panel_aggregates<-Z_scores_nano %>% mutate(
  ...
  IFN.alpha.Siddiqi =  rowMeans(select(Z_scores_nano,
                          cytokine_score_sets$IFN.alpha.cluster.Siddiqi)),
  IFN.beta.Siddiqi =  rowMeans(select(Z_scores_nano,
                          cytokine_score_sets$IFN.beta.cluster.Siddiqi)),
  IFN.gamma.Siddiqi =  rowMeans(select(Z_scores_nano,
...
Cytokine_Panel_geomeans<-nanostring_biox %>% mutate(
  ...
  TNF.related.Geomean = exp(rowMeans(log(select(nanostring_biox,
                          cytokine_score_sets$TNF)))),
  IFN.alpha.Siddiqi.Geomean = exp(rowMeans(log(select(nanostring_biox,
                          cytokine_score_sets$IFN.alpha.cluster.Siddiqi)))),
  IFN.beta.Siddiqi.Geomean = exp(rowMeans(log(select(nanostring_biox,
                          cytokine_score_sets$IFN.beta.cluster.Siddiqi)))),
  IFN.gamma.Siddiqi.Geomean = exp(rowMeans(log(select(nanostring_biox,
                          cytokine_score_sets$IFN.gamma.cluster.Siddiqi))))
  )
...
cytokine_scores <- Cytokine_Panel_aggregates%>%
  select(ID,timepoint,
        ...
        IFN.beta.Siddiqi,
        IFN.gamma.Siddiqi)%>%
  merge( select(Cytokine_Panel_geomeans,
                 ID,timepoint,
...
for (schema in schema_names){
  nam<-paste(as.character(schema),".data", sep="") 
...
vo2max.data <- read_excel(path=
                            here("Data",
                                 ...
                                 "_e_visit__vo__max_protokol.data .xlsx"))%>%
  mutate(
...
```{r}
SF_36<-questionnaires.data |>
  select(id,timepoint, treatment,sf_36_1:sf_36_11_d)
...
SF_36_component_scores <- SF.36.Zscores %>% 
  select(id,timepoint,physical.component.score,mental.component.score)

SF_36_recoded2<-SF_36_recoded %>% left_join(SF_36_component_scores, join_by(id, timepoint))

SF_36_aggregates <- SF_36_recoded2 %>% select(id, timepoint, 
...
Bloodsample.wide.no.OGTTorAcute <- Bloodsample.wide %>% 
...
table1.dexa<- `_c_visit__anthropometrics_and_dxa.data` %>% 
...
if(QC){
 data_vo2max_QC <- read_excel(
  ...
  data.table1<-  data.table1.QC %>% mutate(
    ...
    vo_2_max = case_when(
      easytrialQualityTest == T~ vo_2_max,
      T ~ NA_real_
    ),
    hrmax = case_when(
      easytrialQualityTest == T~ hrmax,
      T ~ NA_real_
    )
  )  
  }
...
```{r}
if(PP){
  data.table1.wide.ppvsviolator <- data.table1.wide
  data.table1.wide <- data.table1.wide|>filter(PP)
}
...
list_normal_dist <- c(
  ...
  "Energy_kJ_mean_baseline", "Energy_intake_change", "m_1_2_baseline", "m_3_4_baseline", "m_5_12_baseline")
...
t1a.abbr.flex%>%save_as_docx( values = NULL, 
        path=here("Output","Tables",
                  paste0(
                    "table1.abbreviated",
...
t1a.m12elevated.abbr%>%save_as_docx( values = NULL, 
    path=here("Output","Tables",
              paste0(
...
```{r}
data.table1<-
  data.table1 %>%mutate(timepoint = factor( timepoint, levels = c("screening", "baseline", "followup")),
                        treat = case_when(
                          timepoint == "followup" ~ treatment,
                          T~"Pingvin"
                        ),
...
data.table1 |>
  filter(!is.na(m_1_2_quantile_at_baseline))|>
  ggplot(
  aes( y = aerobiccapacity, x = treat_time, fill = m_1_2_quantile_at_baseline)) +
    geom_boxplot()+
    geom_point(position = "jitter", aes( colour = m_1_2_quantile_at_baseline))
...
data.table1 |>
  ...
  ggplot(
  aes( y = FSS, x = treat_time, fill = m_1_2_quantile_at_baseline)) +
    geom_boxplot()+
