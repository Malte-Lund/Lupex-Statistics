...
```{r}
Followup.directory <- 
  "L:/AuditData/LupEx/TMF/10 Kildedata/VO2max/Followup"
Baseline.directory <-
  "L:/AuditData/LupEx/TMF/10 Kildedata/VO2max/Baseline"
Screening.directory <-
  "L:/AuditData/LupEx/TMF/10 Kildedata/VO2max/Screening"
```
...
out_data <- data.frame( 
                ...
                maxRQ = NA_real_,
                O2_at_vent_threshold=NA_real_,
                vent_threshold = NA_real_
                )

if(vslope){
  out_data <- out_data %>%
    mutate(
      vslope_vent_threshold = NA_real_,
      O2_vent_thresh = NA_real_,
      CO2_vent_tresh = NA_real_ 
    )
}
...
#Read date, height and weight straight from the excel sheet (should probably have been just one call to read)
...
write_xlsx(x=df,
  ...
  borg<-metadata%>%
    dplyr::filter( id==curr_id & timepoint==curr_timepoint)%>%
    pull(metadata_borg)  
  ...
  out_data$O2_at_vent_threshold[out_data$from_file==filename] = df$VO2[min(which(df$next_n_overVT==6), na.rm=T)]
  
  out_data$vent_threshold[out_data$from_file==filename] = 
    df$VO2[min(which(df$next_n_overVT==6))] / max(df$vo2.30.sec.sum, na.rm =T)*100
...
if(df$time.seconds[match(
  max(df$aerob30secsum, na.rm =T), 
  df$aerob30secsum)] < 
   shortest.analysed.test){
...
#### --- VSLOPE --------------------------------------------------
if(vslope){ #using the new and improved segmented package
  if(is.na(all(df$next_n_overVT))){
    out_data$vslope_vent_threshold[out_data$from_file==filename] <- NA_real_

  } else if (max(df$next_n_overVT,na.rm=T)<6){
    out_data$vslope_vent_threshold[out_data$from_file==filename] <- NA_real_
    ...
    df_vslope<-
      df%>%
      dplyr::filter(
        VO2 >= quantile(df$VO2, vslope_keep[1], na.rm =T))%>%
      dplyr::filter(
        VO2 <= quantile(df$VO2, vslope_keep[2], na.rm =T)
      )
...
O2_vent_thresh<-confint(seggy)[1]
O2_vent_thresh_lowCI <- confint(seggy)[2]
O2_vent_thresh_uppCI <- confint(seggy)[3]

CO2_vent_tresh<-
  predict(seggy, 
          newdata = data.frame(VO2=confint(seggy)[1]))
CO2_vent_thresh_lowCI <- 
  predict(seggy, 
          newdata = data.frame(VO2=confint(seggy)[2]))
CO2_vent_thresh_uppCI <- 
  predict(seggy, 
          newdata = data.frame(VO2=confint(seggy)[3]))

    #save data.
  out_data$vslope_vent_threshold[out_data$from_file==filename] <- 
    100*O2_vent_thresh/out_data$VO2max[out_data$from_file==filename]
  out_data$O2_vent_thresh[out_data$from_file==filename] <- O2_vent_thresh
  out_data$O2_vent_thresh_lowCI[out_data$from_file==filename] <- O2_vent_thresh_lowCI
  out_data$O2_vent_thresh_uppCI[out_data$from_file==filename] <- O2_vent_thresh_uppCI
  
  out_data$CO2_vent_tresh[out_data$from_file==filename] <- CO2_vent_tresh
  out_data$CO2_vent_thresh_lowCI[out_data$from_file==filename] <- CO2_vent_thresh_lowCI
  out_data$CO2_vent_thresh_uppCI[out_data$from_file==filename] <- CO2_vent_thresh_uppCI
...
png(
  file =  
      paste0(outpath , "/graphics/","graph_Vslope_",curr_timepoint,"_",curr_id,graph_filetype),
...
plot(seggy, res=F,
     ...
     ylim =c(0,1.1*max(df_vslope$VCO2)), 
     xlim = c(0,1.1*max(df_vslope$VO2)))+
...
#  graphic2<-df %>%
 #   ggplot(aes(x=VCO2, y = VO2, colour=coloring)) + 
  #  geom_point()+
#    stat_smooth(method="lm", data = df%>%dplyr::filter(coloring !="neither"), fullrange=T)+
 #   scale_color_manual(
  #    values = c("hotpink","grey","navy")
   # )+
  #  stat_regline_equation()
  #ggsave(filename = paste0(outpath , "/graphics/","graph_Vslope_",curr_timepoint,"_",curr_id,graph_filetype),
  #       plot = graphic2)
  #I wanted to output a list, it didn't really work. Especially with the ggplot objects
#curr_graph <- list(data.frame(timepoint=curr_timepoint,id=curr_id, graph=graphic))
#out_list<-append(out_list,curr_graph)
  ...
  #### --- VSLOPE2 --------------------------------------------------
  if(vslope2)if(is.na(all(df$next_n_overVT))){
    out_data$vslope_vent_threshold[out_data$from_file==filename] <- NA_real_
    ...
    df_vslope <- NULL #TODO add this
  }


#this is where the main loop going through every individual files ends
}
...
```{r}
data.followup.poole <- 
  getandtestvo2max(directory = Followup.directory, 
                   metadata = VO2_metadata, 
                                        outpath = here("Data","VO2FromR","poole"),
                                        method="poole", graph = T, models = T)
data.baseline.poole <- 
  getandtestvo2max(Baseline.directory, metadata = VO2_metadata, outpath = here("Data","VO2FromR","poole"),method="poole", graph = T, models = T)
...
#TODO run this again!
data.followup <- getandtestvo2max(Followup.directory, metadata = VO2_metadata, outpath = here("Data","VO2FromR"),method = "MLA", graph = T, models = T, vslope = T)
data.baseline <- getandtestvo2max(Baseline.directory, metadata = VO2_metadata, outpath = here("Data","VO2FromR"),method = "MLA", graph = T, models = T, vslope = T)
data.screening <- getandtestvo2max(Screening.directory, metadata = VO2_metadata, outpath = here("Data","VO2FromR"),method = "MLA", graph = T, models = T, vslope = T)
```
...
##Manual check of EasyTrial Quality So I went through all of the ones with a NA or False value in the easytrial-quality-test parameter to check whether they were insufficient.
...
#So the tests that cannot be gathered from the above are: 
#Lup010 followup, Lup011 Baseline, Lup015 Baseline, Lup023 Baseline, Lup032 baseline & Lup020 Baseline
#We can read their data from the easy-trial files, and check whether they had sufficient tests there.

vo2max.data.full <- read_excel(path =  here("Data","vo2max.xlsx"))

vo2max.data.not_auto_read <-
  vo2max.data.full %>%
  left_join(data_vo2max_QC%>%
              dplyr::select(easytrialQualityTest, id, timepoint), 
            by=join_by(id, timepoint))%>%
  dplyr::filter(
    is.na(easytrialQualityTest)
  )%>%mutate(
    aerobiccapacity = kondital,
    easytrialQuality_score = 
    case_when(
      grepl("0", er_vo_2_max_nået) ~ 1,
      T ~ 0
    ) +
      case_when(
      grepl("1", er_vo_2_max_nået) ~ 1,
      T ~ 0
    ) +
      case_when(
      grepl("late", er_vo_2_max_nået) ~ 1,
      T ~ 0
    ),
    easytrialQualityTest = case_when(
      easytrialQuality_score > 1 ~ T,
      T ~ F
    ))
...
```{r}
##More complext version of MLA-test
...
if(vslope){ #using the new and improved segmented package
  if(is.na(all(df$next_n_overVT))){
    ...
    require(segmented)
  quanty <- quantile(
    df$VO2, c(0.02,0.49,0.51,0.98), na.rm = T
  )
  quanty2 <- quantile(
    df$VCO2, c(0.02,0.49,0.51,0.98), na.rm = T
  )
  
  
  df<-df%>%mutate(
    vo2_quanty = case_when(
      VO2 < quanty[2] ~ 
        case_when( VO2 > quanty[1] ~
                                     "low",
                                   T~"neither"),
      VO2 < quanty[4] ~ 
        case_when(VO2 > quanty[3] ~ 
                                    "high",
                                  T~"neither"),
      T ~ "neither"
    ),
    vco2_quanty = case_when(VCO2 < quanty2[2] ~ 
        case_when( VCO2 > quanty2[1] ~
                                     "low",
                                   T~"neither"),
      VCO2 < quanty2[4] ~ 
        case_when(VCO2 > quanty2[3] ~ 
                                    "high",
                                  T~"neither"),
      T ~ "neither"
    ),
    coloring = factor(case_when(
      vo2_quanty == "low" ~ case_when( vco2_quanty =="low" ~ "low",
                                       T~"neither"),
      vo2_quanty == "high" ~ case_when( vco2_quanty =="high" ~ "high",
                                        T~"neither"),
      T ~ "neither"), 
      levels = c("low","neither","high"))
  )
  ...
  df_high_o2 <- df%>%dplyr::filter(
    coloring == "high"
  )
  if(nrow(df_low_o2)<3){next}
  if(nrow(df_high_o2)<3){next}
  #Fit lines
  lm_low_o2 <- lm(
    VCO2 ~ VO2,
    data = df%>%dplyr::filter(coloring == "low")
  )
  lm_high_o2 <- lm(
    VCO2 ~ VO2,
    data = df%>%dplyr::filter(coloring == "high")
  )
  
  #Extract coefficients
  a1 <- lm_low_o2$coefficients[2]
  b1 <- lm_low_o2$coefficients[1]
  a2 <- lm_high_o2$coefficients[2]
  b2 <- lm_high_o2$coefficients[1]
  #Calculate X of intersection
  O2_vent_thresh <- as.numeric((b2 - b1)/(a1-a2))
  #Y of intersection
  CO2_vent_tresh <- as.numeric(a1*O2_vent_thresh+b1)
  #save data.
  out_data$vslope_vent_threshold[out_data$from_file==filename] <- 
    100*O2_vent_thresh/out_data$VO2max[out_data$from_file==filename]
  out_data$O2_vent_thresh[out_data$from_file==filename] <- O2_vent_thresh
