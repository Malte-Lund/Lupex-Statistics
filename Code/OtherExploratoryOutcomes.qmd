...
summary.no.sex.DLCO.IFN1.lm<-summary(no.sex.DLCO.IFN1.lm)
  summary.no.sex.DLCO.IFN1.lm
  
no.sex.DLCO.IFN1<-
  lfu.data|>
  dplyr::filter(timepoint == "baseline")|>
  ggplot(aes(x = ifn_1_netto, y = dlc_oc_rå))+
  geom_point()+
    geom_smooth(method = "lm")+
  labs(
    x = "IFN-1 score",
  ...
  ggplot2::annotate(
      ...
      y = 8,
      label = paste(
        "slope = ", round(summary.no.sex.DLCO.IFN1.lm$coefficients[3,1],2),
        " (95% CI", round(confint(no.sex.DLCO.IFN1.lm)[3,1],2),
        ";",        round(confint(no.sex.DLCO.IFN1.lm)[3,2],2),"\n ",
        "p = ",     round(summary.no.sex.DLCO.IFN1.lm$coefficients[3,4],3),")"
      )
    )
no.sex.DLCO.IFN1  
no.sex.DLCO.IFN1%>%
  ggsave(
    filename = here("Output", "Graphics","DLCOvsIFN1.jpg"),
    plot = .
  )
