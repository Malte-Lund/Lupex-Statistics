...
data.table1 <-
  ...
  mutate(ifn_1_netto_elevated = 
    ...
    exercise_duration = factor(
      exercise_duration, levels = c(
        "Never Exercise",
    ...
    weekly_exercise_sessions = factor(
...
label(data.table1$smoking_status) <- "Smoker"
label(data.table1$pack_years) <- "Pack Years"
units(data.table1$pack_years) <- "Years"
...
data.table1.wide <- 
  ...
  values_from = c(aerobic_capacity, FSS, vo_2_max, 
                  ...
                  weekly_exercise_sessions, exercise_intensity, exercise_duration,
                  m_1_2:bmi))%>%
  relocate(
    id, treatment, sex, 
...
label(data.table1.wide$weekly_exercise_sessions_baseline) <- "Weekly Exercise Sessions - before intervention"
label(data.table1.wide$exercise_intensity_baseline) <- "Exercise Intensity - before intervention"
label(data.table1.wide$exercise_duration_baseline) <- "Exercise Duration - before intervention"
...
label(data.table1.wide$smoking_status_baseline)  <- "Smoking status"
units(data.table1.wide$pack_years_baseline)  <- "years"
label(data.table1.wide$pack_years_baseline) <- "Smoking pack years"
...
label(data.table1.wide$ifn_1_netto_elevated	) <- "IFN-1 Z-score"
units(data.table1.wide$ifn_1_netto_elevated	) <- "SD"
...
t1.medication.slicc.stratified<-
  data.table1.wide%>%
  filter(!is.na(ifn_1_netto_elevated))%>%
  table1(x= ~ 
              prednisolone_baseline+
            ...
            #render.missing = T,
            data = .)
t1flex(t1.medication.slicc.stratified)%>%
  save_as_docx( values = NULL,
            path=here("Output","Tables",
                      paste0(
                        "table1_medication_slicc_stratified",
                  ifelse(mITT,"_mITT",ifelse(PP, "_PP", "")),
                  ifelse(QC,paste0("_QC",
                      ifelse(Easytrial, "Easytrial",
                           ifelse(MLA,"MLA", 
...
if(PP){
 ...
 # data.table1.wide.ppvsviolator <- tidyr::unite(
  #  PP_char_treat , 
   #   c("treatment", "PP_char"),
    #remove = F
    #)
  
data.table1.wide.ppvsviolator.IFNnomiss<-
  data.table1.wide.ppvsviolator %>% 
  dplyr::filter(!is.na(ifn_1_netto_elevated))%>%
  mutate(
  pp_char_treat_ifn = 
    factor(paste(treatment, PP_char, ifn_1_netto_elevated, sep = " - "
),
levels = c(
  "control - Followed protocol - Normal",
  "control - Followed protocol - Slightly Elevated" ,
  "control - Followed protocol - Highly Elevated", 
  "exercise - Did not follow protocol - Normal",
  "exercise - Did not follow protocol - Slightly Elevated",
  "exercise - Did not follow protocol - Highly Elevated",
  "exercise - Followed protocol - Normal",
  "exercise - Followed protocol - Slightly Elevated",
  "exercise - Followed protocol - Highly Elevated" 
)))
  
  
t1a.abbr.ProtocolVsViolators.IFN<-
    table1::table1(x= ~ sex +
              ethnicity_baseline+
              age +
              weight_baseline+
              ...
              #render.categorical = ,
            #render.missing = T,
            data = data.table1.wide.ppvsviolator.IFNnomiss,
          overall =F)
...
Write different table 1s for the papers (II and III) looking at subpopulations ###Table 1

```{r}
table1.pet.raw <- read_excel(
  here("data","PET-CT","PBV_LUPEX.xlsx"  ))

colnames(table1.pet.raw)<-
  colnames(table1.pet.raw)%>%to_snake_case()
...
table1.pet <- 
  ...
  mutate(
    timepoint = 
      case_when(
      timepoint == "FU" ~ "followup",
      timepoint == "Baseline" ~ "baseline",
      T ~ to_snake_case(timepoint
    )),
    pet = T,
    id = to_snake_case(id)
  )%>%
  dplyr::select(
    -weight, -height,-bmi, -`25`, -gruppe
  )


data.table1.pet<- data.table1 %>%
  left_join(
    table1.pet, by=c("id","timepoint"))%>%
  dplyr::filter(
    pet
  )

#pivot wider is it needed 
#TODO determine this
data.table1.pet.wide <- 
  data.table1.pet %>%
  pivot_wider(
  id_cols=c(id, treatment, sex, mITT, age, pet, 
            #etnisk_oprindelse,etnicitet_anden,
            PP, attendance, 
            attendance_sufficient, 
            unsuccesful_exercise, 
  ...
  values_from = c(aerobic_capacity, FSS, vo_2_max, 
                  borg_skala_efter_test, hrmax, watt_max, 
                  m_1_2:bmi,
                  scanningsdato:kommentar))%>%
  relocate(
    id, treatment, sex, pbv_r_22_baseline, pbv_r_22_followup,   pbv_r_24_baseline, pbv_r_24_followup
  )

data.table1.pet.wide<-data.table1.pet.wide%>% 
       #unnest(cols = id:bmi_followup)  %>%
       #type.convert(as.is = TRUE)%>%
  mutate(
    SLE_Duration = lubridate::as.period(lubridate::interval(sle_start_dato_baseline,dato_dxa_baseline)),
