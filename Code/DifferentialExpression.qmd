...
group<-read_excel(path=here("data","RandomizationKey_pingvin_søløve.xlsx"))%>%dplyr::select(ID,Allocation)%>%
  mutate(treatment=Allocation)
...
Linear mixed model on principle components
```{r}
