...
summary(model25.main.fatigue_as_FSS.normalIFN.lmm, level = 0.95)
confint(model25.main.fatigue_as_FSS.normalIFN.lmm, level = 0.95)
plot(model25.main.fatigue_as_FSS.normalIFN.lmm, type=c("qqplot"))
plot(model25.main.fatigue_as_FSS.normalIFN.lmm, type= "fit", 
     color = "treatment", at = data.frame(sex = "F"),
     obs.alpha = 0.2,
     ci = F)
plot(model25.main.fatigue_as_FSS.normalIFN.lmm, type=c("scatterplot"))
```

### Fatigue Model two-level IFN score
...
model.IFN1_high_low.FSS.lmm<-lmm(
  formula = 
    fatigue.mean ~ 
    timepoint+
    sex+
...
plot(model.IFN1_high_low.FSS.lmm, type = "qqplot")
plot(model.IFN1_high_low.FSS.lmm, type = "scatterplot")


summary(model.IFN1_high_low.FSS.lmm, level = 0.95)

model.IFN1_high_low.FSS.lmm.plot<-
  plot(model.IFN1_high_low.FSS.lmm, type = "fit", at = data.frame( sex = "F"), color = "IFN1_treatment")


IFN1_high_low.prettyplot<-
  model.IFN1_high_low.FSS.lmm.plot$plot + 
  scale_color_manual(
  values = c("red","blue","hotpink","purple")
)+ scale_fill_manual(
  values = c("red","blue","hotpink","purple")
)+theme_bw()+
  labs(x = "", y = "FSS", color = "Treatment and IFN-1")+
  guides(fill="none")



model.IFN1_high_low.FSS.lmm.plot_individual_data<-
  plot(model.IFN1_high_low.FSS.lmm, type = "fit", at = data.frame( sex = "F"), color = "IFN1_treatment", ci = F, obs.alpha = 0.25)

Individual_data_IFN1_high_low.prettyplot<-
  model.IFN1_high_low.FSS.lmm.plot_individual_data$plot + 
  scale_color_manual(
  values = c("red","blue","hotpink","purple")
)+ scale_fill_manual(
  values = c("red","blue","hotpink","purple")
...
model.IFN1_high_low.FSS.lmm.plot_facet<-
  plot(model.IFN1_high_low.FSS.lmm, type = "fit", at = data.frame( sex = "F"), color = "treatment", facet = ~IFN1_high_low, ci = F, obs.alpha = 0.25)

model.IFN1_high_low.FSS.lmm.facet_pretty.plot<-
  model.IFN1_high_low.FSS.lmm.plot_facet$plot+
  theme_bw()+
  labs(x = "", y = "FSS", color = "Treatment")
model.IFN1_high_low.FSS.lmm.facet_pretty.plot

model.IFN1_high_low.FSS.lmm.facet_pretty.plot%>%
ggsave(
  filename = here("output","graphics","model.IFN1_high_low.FSS.lmm.facet_pretty.plot.jpg"),
  width = 12
)

Individual_data_IFN1_high_low.prettyplot%>%
ggsave(
  filename = here("output","graphics","model.IFN1_high_low.FSS.lmm.plot_individual_data.jpg"),
  width = 12
)
...
IFN1_high_low.prettyplot%>%
ggsave(
  filename = here("output","graphics","IFN1_high_low.prettyplot.jpg"),
  width = 12
)
...
### Fatigue J williams & IL-6 
```{r}
model.JW.FSS.lmm<-lmm(
  formula = 
    fatigue.mean ~ 
    timepoint+
    sex+
...
plot(model.JW.FSS.lmm, type = "qqplot")
plot(model.JW.FSS.lmm, type = "scatterplot")

summary(model.JW.FSS.lmm, level = 0.95)

model.il6.FSS.lmm<-lmm(
  formula = 
    fatigue.mean ~ 
    timepoint+
    sex+
    timepoint:treat+
    il_6_related+
    il_6_related:timepoint+
    il_6_related:timepoint:treat,
  repetition = ~timepoint|id,
  structure = "UN",
  data = questionnaires.combined.data
)
plot(model.il6.FSS.lmm, type = "qqplot")
plot(model.il6.FSS.lmm, type = "scatterplot")

summary(model.il6.FSS.lmm, level = 0.95)
...
fatigue.data.wide<-
  ...
  dplyr::select(id, treatment, timepoint,
         ...
         il_6_related, il_6_related_geomean)%>% pivot_wider(
  ...
  id_cols = c("id", "treatment", "sex", 
              ...
              "ifn_2_el_sherb", "ifn_2_el_sherb_geomean", 
              "ifn_2_siddiqi", "ifn_2_siddiqi_geomean",
              "il_6_related", "il_6_related_geomean"),
  values_from = c("fatigue.mean"))%>%
  mutate(
  delta_fatigue = followup-baseline)

lm.delta_fatigue_IFN1<-
  fatigue.data.wide %>%
  lm(
    formula =
      delta_fatigue ~ sex + ifn_1_netto + treatment + treatment:ifn_1_netto,
    data =.
  )
summary(lm.delta_fatigue_IFN1)

sum.lm.delta_fatigue_IFN1<-summary(lm.delta_fatigue_IFN1)
...
fatigue_by_IFN <- 
  ...
  annotate(
    ...
    label = paste0("difference of slopes = ", 
                          round(sum.lm.delta_fatigue_IFN1$coefficients[5,1], 2), "FSS per SD (95%CI:",
                          round(confint(lm.delta_fatigue_IFN1)[5,1],2),
                                " to ",
                          round(confint(lm.delta_fatigue_IFN1)[5,2],2),
                                " )") 
    )+
  annotate(
    geom = "text",
    ...
    label = paste0(
      "unadjusted p = ",
      round(sum.lm.delta_fatigue_IFN1$coefficients[5,4], 2)
    )
  )
...
require(patchwork)
fatigue_by_IFN+fatigue_by_IFN_cubic +plot_layout(guides="collect")
...
lab_sex <- as_labeller( function(variable,value){
  return(lab_names[value])
})

 fatigue_by_IFN_facetsex <- fatigue_by_IFN + 
    facet_wrap(~sex, labeller = lab_sex
)
...
ggsave(filename=here("Output","Graphics",paste0(
  ...
  ifelse(PP,"PP",""),
  ".jpg")),
  plot=fatigue_by_IFN_facetsex#fatigue_by_IFN + facet_wrap(~sex, labeller = lab_sex)
  )
...
model.delta.fatigue.sex<-
  lm(delta_fatigue ~ sex + sex:treatment + ifn_1_netto * treatment, data = fatigue.data.wide)
plot(model.delta.fatigue.sex)
summary(model.delta.fatigue.sex)
...
fatigue_by_IL6 <- 
  ggplot(fatigue.data.wide, 
         aes(x = il_6_related, 
             ...
             colour = treatment)) +
  geom_point() +  
  geom_smooth(method="lm", formula = y ~ x, se=T, level = 0.975) + 
        theme_bw() +
  labs(title = "Change in Fatigue by treatment and Il-6 score",
       x = "IL-6 score (SD)",
       y = "\u0394 FSS",
       colour = "Treatment")+
  scale_colour_discrete(name="Treatment",
...
model.delta.fatigue.IL6<-
  lm(delta_fatigue ~ sex + il_6_related * treatment, data = fatigue.data.wide)
plot(model.delta.fatigue.IL6)
summary(model.delta.fatigue.IL6)
...
#### SF-36 Individual groups
```{r}
model.main_effect.SF36_energy.lmm<-lmm(
  formula = energy_fatigue ~ timepoint+treat:timepoint+sex,
  repetition = ~timepoint|id,
  structure = "UN",
  data = questionnaires.combined.data
)

plot(model.main_effect.SF36_energy.lmm, type=c("qqplot"))
plot(model.main_effect.SF36_energy.lmm, type=c("scatterplot"))
summary(model.main_effect.SF36_energy.lmm)
plot.model.main_effect.SF36_energy.lmm<-
  plot(model.main_effect.SF36_energy.lmm, type = "fit", color = "treatment", ci = F, obs.alpha = 0.2, at = data.frame( sex = "F"))

pretty.plot.model.main_effect.SF36_energy.lmm<-
  plot.model.main_effect.SF36_energy.lmm$plot+
  theme_bw()+
  labs(y = "Vitality",
       x = "",
       color = "")
  ...
  ggsave(
    filename = here("output", "graphics","plot.model.main_effect.SF36_energy.lmm.jpg"),
    plot = pretty.plot.model.main_effect.SF36_energy.lmm
  )
#linear.mixed.model.list$model71.main_effect.SF36Physical.lmm <-
 # model.tables(model71.main_effect.SF36Physical.lmm)
...
model.interaction.SF36_energy.lmm<-lmm(
  formula = energy_fatigue ~ 
    timepoint+
    sex+
    timepoint:treat+
    ifn_1_netto+
...
plot(model.interaction.SF36_energy.lmm, type=c("qqplot"))
plot(model.interaction.SF36_energy.lmm, type=c("scatterplot"))
summary(model.interaction.SF36_energy.lmm)
...
summary(model101.main_post_exercise_included.fatigue_as_FSS.lmm, level = 0.95)
...
```
#### Post Hoc Analyses required by reviewer 2

```{r}

model.sexinteraction.fatigue_as_FSS.lmm<-lmm(
  formula = fatigue.mean ~ timepoint+treat:timepoint+sex+sex:treat,
  repetition = ~timepoint|id,
  structure = "UN",
  data = questionnaires.combined.data
)
summary(model.sexinteraction.fatigue_as_FSS.lmm, level = 0.95)
plot(model.sexinteraction.fatigue_as_FSS.lmm, type=c("qqplot"))
plot(model.sexinteraction.fatigue_as_FSS.lmm, type=c("scatterplot"))
plot.model.sexinteraction.fss<-
  plot(model.sexinteraction.fatigue_as_FSS.lmm, type=c("fit"), facet = ~sex, color = "treatment")

plot.sexinteraction.fss<-
  plot.model.sexinteraction.fss$plot+
  theme_bw()+
  labs(
    y="FSS",
    x="",
...
plot.sexinteraction.fss%>%
  ggsave(
  filename = here("output","graphics","fatigue.plot.sexinteraction.jpg"),
  width = 12
)

plot.sexinteraction.model.individual_point.fss<-
  plot(model.sexinteraction.fatigue_as_FSS.lmm, type=c("fit"), facet = ~sex, color = "treatment", ci=F, obs.alpha = .25)

plot.sexinteraction.individual_point.fss<-
  plot.sexinteraction.model.individual_point.fss$plot+
  theme_bw()+
  labs(
    y="FSS",
    x="",
...
plot.sexinteraction.individual_point.fss%>%
  ggsave(
  filename = here("output","graphics","fatigue.plot.individualpoint.sexinteraction.jpg"),
  width = 12
)
...
questionnaires.combined.data<-
  questionnaires.combined.data%>%
  mutate(
    ifn_1_highlow = case_when(
      ifn_1_netto > 2 ~ "high",
      ifn_1_netto <= 2 ~ "low",
      T~NA_character_),
    `IFN-1 and Treatment` =
      paste(
        "IFN:", ifn_1_highlow, "Treatment:", treatment
      )
  )
model.high_low.FSS.lmm<-lmm(
  formula = 
    fatigue.mean ~ 
    timepoint+
    sex+
    timepoint:treat+
    ifn_1_highlow+
    ifn_1_highlow:timepoint+
    ifn_1_highlow:timepoint:treat,
  repetition = ~timepoint|id,
  structure = "UN",
  data = questionnaires.combined.data
)
plot(model.high_low.FSS.lmm, type = "qqplot")
plot(model.high_low.FSS.lmm, type = "scatterplot")

summary(model.high_low.FSS.lmm, level = 0.95)
model.high_low.FSS.lmm.plot<-plot(model.high_low.FSS.lmm, type = "fit", color = "IFN-1 and Treatment",  at=data.frame(sex = "F"))

high_low.FSS.plot.export<-
  model.high_low.FSS.lmm.plot$plot+
  labs(y = "FSS", x = " " )+
  scale_color_manual(values = c("hotpink","red","purple","darkblue"))+
  scale_fill_manual(values = c("hotpink","red","purple","darkblue"))+
  theme_bw()
  
high_low.FSS.plot.export
high_low.FSS.plot.export%>%ggsave(filename=here("Output","Graphics","fatigue_by_high_low_IFN.jpg"),
  plot=.)
```
